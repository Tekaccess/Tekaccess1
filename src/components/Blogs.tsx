    ...
    <section id="blogs" className="bg-slate-50 px-4 py-24 sm:px-6 lg:px-8">
      <div className="relative mx-auto max-w-7xl">
        <div className="mb-16 text-center">
          <span className="inline-block text-[#0A1437] font-bold text-xs tracking-widest uppercase mb-4 px-3 py-1 bg-white rounded-full shadow-sm">
            insights
          </span>
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-[#0A1437] leading-tight tracking-tight mt-4">
            Latest from Our{" "}
            <span className="italic text-slate-500">Knowledge Hub</span>
          </h2>
          <p className="mt-4 text-slate-600 text-lg max-w-2xl mx-auto font-light">
            Stay updated with industry insights, trends, and expert perspectives
            from TekAccess.
          </p>
        </div>
          ...
          {categories.map((cat) => (
            <button
              ...
              className={`whitespace-nowrap px-6 py-2.5 rounded-full text-xs font-bold uppercase tracking-widest transition-all duration-300 border ${
                activeCategory === cat
                  ? "bg-[#0A1437] text-white border-[#0A1437] shadow-lg shadow-[#0A1437]/20"
                  : "bg-white text-slate-500 border-slate-200 hover:border-[#0A1437] hover:text-[#0A1437]"
              }`}
            >
              {cat}
            </button>
