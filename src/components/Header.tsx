...
const Header = () => {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-50">
      {/* Promo Banner */}
      <div className="bg-brand-red py-2 px-4 text-center shadow-lg shadow-brand-red/10">
              ...
              <div className="flex items-center gap-1">
                {navLinks.map((link) => (
                  <button
                    ...
                    className="relative group px-4 py-2 text-sm font-bold text-slate-700 transition-colors duration-300 hover:text-brand-red whitespace-nowrap"
                  >
                    {link.label}
                    <span className="absolute bottom-1 left-1/2 w-0 h-0.5 bg-brand-red transition-all duration-300 group-hover:w-px group-hover:left-1/2 group-hover:scale-x-[20]"></span>
                  </button>
                ))}
              </div>
            </nav>
            ...
            <div className="flex flex-col gap-3">
              {navLinks.map((link) => (
                <button
                  key={link.href}
                  onClick={() => scrollToSection(link.href)}
                  className="flex items-center justify-between rounded-2xl px-6 py-4 text-left text-base font-bold text-slate-600 transition-all duration-300 hover:bg-slate-50 hover:text-brand-red group"
                >
                  {link.label}
                  <div className="h-1.5 w-1.5 rounded-full bg-brand-red opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
                </button>
              ))}
